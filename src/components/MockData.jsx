export const llatest = [
    {    
        id:1,
        header:"Agra,New Delhi",
        description:"city on the banks of the Yamuna river in the Indian state of Uttar Pradesh, about 210 kilometres (130 mi) south of the national capital New Delhi and 378 km west of the state capital Lucknow.",
        tags:['ART','/ December 21 2020'],
        background:require("../pics/agra.jpg")

    },
    {   id:2,
        header:"Hava Mahal,Rajasthan",
        description:"Hawa Mahal, is a palace in Jaipur, India, so named because it was essentially a high screen wall built so the women of the royal household could observe st.",
        tags:['ART','/ December 21 2020'],
        background:require("../pics/hava.jpg")
    },
    {   id:3,
        header:"Leh,The capital of Ladakh",
        description:"Leh, the capital of Ladakh is spreading rapidly in all directions. Leh is situated in a fertile side valley of the Indus river. Since the 17th century Leh has been the capital of the Ladakh region",
        tags:['ART','/ December 21 2020'],
        background:require("../pics/leh.jpg")

    }
]

export const mainBanner = 
    {
      title:"HAVA mahal famous site in jaipur",
      tag:"Mint / December 2020"  ,
      image:require("../pics/hava.jpg")
    }

export const mainHome =[
    {
        headding:"dream city",
        brief:"One of the most popular hill stations in India ,mumbai offers the most magnificent views",
        data:['Mint','/ December 21 2020'],
        backimg:require("../pics/mumbai.jpg")


    },
    {
        headding:"Mumbai",
        brief:"a place for everyone for everything",
        data:['Mint','/ December 21 2020'],
        backimg:require("../pics/mumbai.jpg")


    },
    {
        headding:"ladakh",
        brief:"Ladakh a place of peace",
        data:['Mint','/ December 21 2020'],
        backimg:require("../pics/ladakh.jpg")


    }, 
    {
        headding:"Bombay",
        brief:"mumbai city of dreams",
        data:['Mint','/ December 21 2020'],
        backimg:require("../pics/mumbai2.jpg")


    }
]
export const mainMiddle=[
    {
        number:"2",
        toptext:"Ladakh ",
        midtext:['','/ December 21 2020'],
        textimg:require("../pics/ladakh.jpg")
    },
    {
        number:"3",
        toptext:"Mumbai Maharashtra",
        midtext:['','/ December 21 2020'],
        textimg:require("../pics/mumbai.jpg")
    },
    {
        number:"4",
        toptext:"Mumbai,Maharashtra",
        midtext:['','/ December 21 2020'],
        textimg:require("../pics/mumbai.jpg")
    }
]

export const mainPage=[
    {
        titlehead:"Follow your passion and Dreams as well",
        deschead:"Mumbai, formerly Bombay, city, capital of Maharashtra state, southwestern India..",
        taghead:["TECH","/ TODAY AT 11:54"]

    },
    {
        titlehead:"Follow your passion and Dreams as well",
        deschead:"Mumbai, formerly Bombay, city, capital of Maharashtra state, southwestern India. It is the country’s financial and commercial centre and its principal port on the Arabian Sea. in the world. It was built on a site of ancient settlement, and it took its name from the local goddess Mumba—a form of Parvati, the consort of Shiva, one of the principal deities of Hinduism",
        taghead:["STYLE","/ December 21 2020"]

    },
    {
        titlehead:"Follow your passion and Dreams as well",
        deschead:"Mumbai, formerly Bombay, city, capital of Maharashtra state, southwestern India..",
        taghead:["TECH","/ December 21 2020"]

    }
]

export const mainBolly = [
    {
      bollyhead:"Katrina kaif",
      bollydesc:"A role model for generations to come,Katrina kaif  is hailed as the greatest Bollywood superstar of all time. "  ,
      bollytags:['Mint','/ December 21 2020'],
      bollyimg:require("../pics/kat.jpg")

    },
    {
        bollyhead:"a perfect COUPLE",
        bollydesc:" Katrina Kaif, who tied the knot with Vicky Kaushal in December 2021, opens up about how her life has changed  "  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/katvicky.jpg")
  
    },
    {
        bollyhead:"Big B",
        bollydesc:" Amitabh Bachchan, who made his Bollywood debut in 1969 with Saat Hindustani, the movie Goodbye. . "  ,
        bollytags:['Mint','/December 01 2021'],
        bollyimg:require("../pics/bigb.jpg")
  
    },
    {
        bollyhead:"Sushant Sigh Rajput",
        bollydesc:"2006–2011: Early career and television During his course at the Delhi College of Engineering,' " ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/ssr.jpg")
  
    },
    {
        bollyhead:"Rajkumar Rao",
        bollydesc:"When you think of comedy and Bollywood, this superstar's name is the one that pops up in mind."  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/rajkumar.jpg")
  
    },
    {
        bollyhead:"Bhramhastra ",
        bollydesc:"Indian Hindi-language fantasy action-adventure film written and directed by Ayan Mukerji ."  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/bhr.jpg")
  
    },
    {
        bollyhead:"Ramsetu",
        bollydesc:"Ram Setu (transl. Rama's bridge) is an upcoming Akshay Kumar, Jacqueline Fernandez, Nushrratt Bharuccha "  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/ramsetu.jpg")
  
    }

]
export const mainSecond=[
    {
        secnum:"2",
        sectitle:"'upcoming movie of Akshay'",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/ramsetu.jpg")
    },
    {
        secnum:"3",
        sectitle:"'bhramhastra succesful or not'",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/bhr.jpg")
    },
    {
        secnum:"4",
        sectitle:"'fav actor of industry'",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/sid.jpg")
    }
]

export const mainTech = [
    {
      bollyhead:"Information Technology",
      bollydesc:"Information technology (IT) is the use of any computers, storage"  ,
      bollytags:['Mint','/ December 21 2020'],
      bollyimg:require("../pics/it.jpg")

    },
    {
        bollyhead:"Automation Technology",
        bollydesc:"Automation Testing is a software testing technique that performs"  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/auto.jpg")
  
    },
    {
        bollyhead:"Health Technology",
        bollydesc:"Medical technologies can save lives, improve health and  "  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/medical.jpg")

  
    },
    {
        bollyhead:"Electronics Technology",
        bollydesc:"Computers, smartphones, digital camera, RADAR (Radio Detection And Ranging) "  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/auto.jpg")
  
    },
    {
        bollyhead:"Artificial Intelligence",
        bollydesc:"Artificial intelligence was founded as an academic discipline in 1956 "  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/ai.jpg")
  
    },
    {
        bollyhead:"Biotechnology",
        bollydesc:"Biotechnology is the integration of natural sciences "  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/bio.jpg")
  
    },
    {
        bollyhead:"optical fiber Technology",
        bollydesc:"Optical fibre is the technology associated with data transmission using light pulses Mintling along with a"  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/fiber.jpg")
  
    }

]
export const mainThird=[
    {
        secnum:"2",
        sectitle:"Automation",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/robo.jpg")
    },
    {
        secnum:"3",
        sectitle:"Instrumentation and Control",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/auto.jpg")
    },
    {
        secnum:"4",
        sectitle:"Control Engineering",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/auto.jpg")
    }
]

export const mainHolly = [
    {
      bollyhead:"Angelina geli",
      bollydesc:"Angelina Jolie DCMG is an American actress, filmmaker, and humanitarian. "  ,
      bollytags:['Mint','/ December 21 2020'],
      bollyimg:require("../pics/ag.jpg")

    },
    {
        bollyhead:"Shawn Mendes",
        bollydesc:"A multi-talented person because in the Hollywood industry people "  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/sm.jpg")
  
    },
    {
        bollyhead:"Pop start JB",
        bollydesc:"Teen pop star turned versatile, Grammy-winning hitmaker who has"  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/jb.jpg")

  
    },
    {
        bollyhead:"Charlie puth",
        bollydesc:"American singer who has done many exceptional movies like Titanic,"  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/cp.jpg")
  
    },
    {
        bollyhead:"Tom Cruise",
        bollydesc:"In 1976, if you had told fourteen-year-old Franciscan seminary   "  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/tc.jpg")
  
    },
    {
        bollyhead:"shawn mendes",
        bollydesc:"In 1976, if you had told fourteen-year-old Franciscan  ",
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/sm.jpg")
  
    },
    {
        bollyhead:" justin biber",
        bollydesc:"Teen pop star turned versatile, Grammy-winning hitmaker who has ." ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/jb.jpg")
  
    }

]
export const mainFourth=[
    {
        secnum:"2",
        sectitle:"Teen Pop Star",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/cp.jpg")
    },
    {
        secnum:"3",
        sectitle:"Famous Singer",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/sm.jpg")
    },
    {
        secnum:"4",
        sectitle:"Famous singer",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/jb.jpg")
    }
]

export const mainFitness = [
    {
      bollyhead:"Stay Healthy",
      bollydesc:"Dr Ambrish Mithal, Chairman and Head of Diabetes and Endocrinology  "  ,
      bollytags:['Mint','/ December 21 2020'],
      bollyimg:require("../pics/fitt2.jpg")

    },
    {
        bollyhead:"Do Yoga",
        bollydesc:"This Beginner's Yoga Session Will Make You Feel Pure Joy"  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/fitt3.jpg")
  
    },
    {
        bollyhead:"Stay fit",
        bollydesc:"  Exercise can help prevent excess weight gain or help maintain …"  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/fitt1.jpg")

  
    },
    {
        bollyhead:"stay fit",
        bollydesc:"Exercise combats health conditions and diseases. Worried about heart disease? "  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/fitt4.jpg")
  
    },
    {
        bollyhead:"Benefits OF exercise",
        bollydesc:"Exercise improves mood. Need an emotional lift? Or need to blow off some steam"  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/fitt5.jpg")
  
    },
    {
        bollyhead:"Stay fit",
        bollydesc:"Exercise combats health conditions and diseases. Worried about heart disease? "  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/fitt6.jpg")
  
    },
    {
        bollyhead:"worried about health?",
        bollydesc:"Exercise combats health conditions and diseases. Worried about heart disease?"  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/fitt4.jpg")
  
    }

]
export const mainFifth=[
    {
        secnum:"2",
        sectitle:"worried about health",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/fitt1.jpg")
    },
    {
        secnum:"3",
        sectitle:" DO Yoga",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/fitt2.jpg")
    },
    {
        secnum:"4",
        sectitle:"DO exercise regularly",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/fitt3.jpg")
    }
]

export const mainFood = [
    {
      bollyhead:"Eggs and meat",
      bollydesc:"Eggs and meat” as it is commonly known in most parts of India, ...."  ,
      bollytags:['Mint','/ December 21 2020'],
      bollyimg:require("../pics/meat.jpg")

    },
    {
        bollyhead:" Vegetables",
        bollydesc:"Green leafy vegetables are already popular in the Indian diet. "  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/veg.jpg")
  
    },
    {
        bollyhead:"Rice plate",
        bollydesc:"While they are not always an option for are excellent sources of protein."  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/rice.jpg")

  
    },
    {
        bollyhead:"burger",
        bollydesc:"sometimes it is fine to eat fast food "  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/burger.jpg")
  
    },
    {
        bollyhead:"Fruits",
        bollydesc:" fruits are great for you. They contain all kinds of minerals."  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/fruits.jpg")
  
    },
    {
        bollyhead:"Juices",
        bollydesc:"juice  are a great source of proteins, calcium, iron and folic acid,cook.."  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/juice.jpg")
  
    },
    {
        bollyhead:"lemon",
        bollydesc:"lemon is flavourful,also renowned for its many healing properties and.."  ,
        bollytags:['Mint','/ December 21 2020'],
        bollyimg:require("../pics/lemon.jpg")
  
    }

]
export const mainSixth=[
    {
        secnum:"2",
        sectitle:"Healthy Foods",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/rice.jpg")
    },
    {
        secnum:"3",
        sectitle:"Healthy Foods",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/veg.jpg")
    },
    {
        secnum:"4",
        sectitle:"Healthy Foods",
        sectags:['','/ December 21 2020'],
        secimg:require("../pics/lemon.jpg")
    }
]

export const bottom = [
    {    
        header:"",
        description:"Tower",
        tag1:'Dmitry Nozhenko',
        tag2:'Jan 28,2019 10 min read',
        background:require("../pics/hava.jpg"),
        imgex:require("../pics/build.jpg")

    },
    {
        header:"Related Reads",
        description:"Pune",
        tag1:'Dmitry Nozhenko',
        tag2:'Jan 28,2019 10 min read',
        background:require("../pics/build.jpg"),
        imgex:require("../pics/pune.jpg")
    },
    {   
        header:"Related Reads",
        description:"Jaipur",
        tag1:'Dmitry Nozhenko',
        tag2:'Jan 28,2019 10 min read',
        background:require("../pics/agra.jpg"),
        imgex:require("../pics/pune2.jpg")

    }
]